...
- **Directive protocol**: Claude agents emit output as stream-json `result` events. Each result is scanned line-by-line: lines starting with `{` are parsed as directives and intercepted (not posted to Discord); all other lines are collected and posted as content. `{"type":"restart"}` posts a notice to Discord then calls `os.Exit(0)`; Docker's `restart: unless-stopped` brings the process back up. `{"type":"done"}` is a no-op (the `result` event already signals turn completion).
